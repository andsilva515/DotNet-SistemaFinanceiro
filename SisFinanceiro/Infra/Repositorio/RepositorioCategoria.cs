...
namespace Infra.Repositorio
{
    ...
    {
        private readonly DbContextOptions<ContextBase> _OptonsBuilder;
        public RepositorioCategoria()
        {
            _OptonsBuilder = new DbContextOptions<ContextBase>();
        }
        public async Task<IList<Categoria>> ListarCategoriasUsuario(string emailUsuario)
        {
            using (var banco = new ContextBase(_OptonsBuilder))
            {
                return await
